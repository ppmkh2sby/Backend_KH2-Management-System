...
public interface IFaceRecognitionClient
{
    ...
    Task<FaceEnrollmentResult> EnrollAsync(Guid userId, IReadOnlyCollection<FaceImage> images, CancellationToken cancellationToken);
    Task<FaceOpenerVerificationResult> VerifyOpenerAsync(string providerProfileId, FaceImage image, CancellationToken cancellationToken);
...
public sealed record FaceRecognitionResult(string? ProviderProfileId, decimal? Confidence, int FaceCount, string? Reason);
